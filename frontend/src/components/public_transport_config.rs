...
pub fn public_transport_config_component(props: &PublicTransportConfigProps) -> Html {
    ...
    html! {
        <div class="flex flex-col gap-2">
            <input type={"text"} onchange={on_changed_from} value={props.config.from.clone()} placeholder={"From"} class="rounded-sm placeholder:pl-1"/>
            <input type={"text"} onchange={on_changed_to} value={props.config.to.clone()} placeholder={"To"} class="rounded-sm placeholder:pl-1"/>
        </div>
    }
}
